...
class ActivationKeyController extends Controller
{
    ...
    public function activateKey($activation_key)
    {
        ...
        if (Auth::check()) {
            if (auth()->user()->activated) {
                ...
                return redirect()->route('login')
                    ->with('message', 'Tài khoản của bạn đã được kích hoạt.')//Your email is already activated.
                    ->with('status', 'success');
            }
        ...
        if (empty($activationKey)) {
            ...
            return redirect()->back()
                ->with('message', 'Mã kích hoạt dường như không hợp lệ')//được cung cấp dường như không hợp lệ
                ->with('status', 'warning');
        ...
        return redirect()->route('login')
            ->with('message', 'Tài khoản của bạn đã được kích hoạt. Bây giờ bạn có thể đăng nhập!')//You successfully activated your email! You can now login
            ->with('status', 'success');
    ...
    public function resendKey(Request $request)
    {
        ...
        if (empty($user)) {
            return redirect()->back()
                ->with('message', 'Email này chưa đăng ký!')//We could not find this email in our system
                ->with('status', 'warning');
        }
         
        if ($user->activated) {
            return redirect()->route('login')
             ->with('message', 'Tài khoản của bạn đã kích hoạt rồi.')//This email address is already activated
                ->with('status', 'success');
        }
        ...
        return redirect()->back()
            ->with('message', 'Mã kích hoạt đã được gửi tới email của bạn!')//The activation email has been re-sent.
            ->with('status', 'success');
    }
}